    ...
    createDM = async () => {
        try {
            ...
            if (e.response.status === 400) {
                this.setState({
                    createDMSuccess: false,
                    DMerrorMessage: e.response.data.message,
                });
            } else
                this.setState({
                    createDMSuccess: false,
                    DMerrorMessage:
                        "Error unable to create Dm. Check console log",
                });
            console.log(e);
        }
    };
    ...
    handleUserSuggestion = async (selectedUser) => {
        try {
            let res = await axios.post(
                process.env.REACT_APP_HOST + "/api/dms/checkdmexists",
                {
            ...
            if (e.response.status === 400) {
                ...
                this.setState({
                    createDMSuccess: false,
                    DMerrorMessage:
                        "Error unable to verify Dm existence. Check console log",
                });
            console.log(e);
        }
    ...
    render() {
        ...
        if (this.state.userId && this.state.messages && this.state.dms) {
            messages = this.state.messages.map((msg, dmsIndex) => {
                return (
                    <div className="message" key={dmsIndex}>
                        ...
                        <div className="messageContent">
                            <div className="messageHeader">
                                ...
                                <p className="messageDate">
                                    {dateParser(msg.date, "ddd h:mm a")}
                                </p>
                            </div>
            ...
            dms = this.state.dms.map((dm) => {
                ...
                let recipient = dm.members.filter(
                    (member) => member._id !== this.state.userId
                );
                recipient = recipient[0];

                // update classname of selected dm
                let classname =
                    dm._id === this.state.dmId ? "dmItem selectedDm" : "dmItem";
        ...
        return (
            <div className="container">
                ...
                <div className="container d-flex justify-content-center">
                    ...
                    <p className="display-6">Welcome {this.state.name}</p>
                </div>
                <div className="row">
                    <div className="col-2">
                        <button
                            ...
                            onClick={() =>
                                this.setState({
                                    showNewDM: !this.state.showNewDM,
                                    createDMSuccess: true,
                                })
                            }
                        >
                    ...
                    <div
                        className="col-1"
                        style={{
                            display: this.state.showNewDM ? "block" : "none",
                        }}
                    >
                        <label for="inputRecipient" className="col-form-label">
                            To:
                        </label>
                    </div>
                    <div
                    ...
                    >
                        <UserSearchComponent
                            ...
                            onSuggestionClick={(selectedUser) =>
                                this.handleUserSuggestion(selectedUser)
                            }
                            filter={(user) => {
                    ...
                    >
                        <button
                            className={
                                this.state.createDMSuccess
                                    ? "btn btn-success"
                                    : "btn btn-success disabled"
                            }
                            onClick={this.createDM}
                        >
                            Add
                        </button>
                    </div>
                ...
                <div className="row">{createDMerrorMsg}</div>
                <div className="row">
                    <div className="dmsSidebar col-2">{dms}</div>
                    <div className="col-10">
                        ...
                        <div className="messageBox d-flex">
                            <textarea
                                className="dmMessageBox form-control"
                                onChange={(e) => this.updateInput("message", e)}
                                value={this.state.message}
                                placeholder="Enter your DM here..."
                            />
                            {/* <input onChange={(e) => this.updateInput('message', e)} value={this.state.message} placeholder= "Enter your DM here..." /> */}
                            <button
                                className="dmMessageButton btn btn-success"
