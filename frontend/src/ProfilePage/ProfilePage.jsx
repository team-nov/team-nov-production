...
class ProfilePage extends Component {
    ...
    componentDidMount() {
        axios
            ...
            .then((res) => {
                ...
                document.getElementsByName("typeOfUser")[0].value =
                    this.state.typeOfUser;
            });

        axios
            .get(process.env.REACT_APP_HOST + "/api/interests/")
            .then((res) => {
                this.setState({
        ...
        axios.get(process.env.REACT_APP_HOST + "/api/company/").then((res) => {
            this.setState({
    ...
    render() {
        ...
        for (let i = 0; i < this.state.allInterests.length; i++) {
            ...
            if (
                this.state.interests.indexOf(
                    this.state.allInterests[i].name
                ) === -1
            ) {
                checklist[i] = (
                    <div>
                        <input
                            type="checkbox"
                            name={this.state.allInterests[i].name}
                            value={this.state.allInterests[i]._id}
                            onChange={this.addInterest}
                        ></input>
                        <label
                            class="checklistLabels"
                            for={this.state.allInterests[i].name}
                        >
                            {this.state.allInterests[i].name}
                        </label>
                    </div>
        ...
        for (let i = 0; i < this.state.allCompanies.length; i++) {
            // console.log(this.state.allInterests[i]);
            let allCompaniesInfo = {
                company: this.state.allCompanies[i].company,
                id: this.state.allCompanies[i]._id,
            };
            console.log(allCompaniesInfo);
            console.log(this.state.team);
            let pos = this.state.team
                .map(function (e) {
                    return e.id;
                })
                .indexOf(allCompaniesInfo.id);
            if (pos === -1) {
                company_checklist[i] = (
                    <div>
                        <input
                            type="checkbox"
                            name={this.state.allCompanies[i].company}
                            value={this.state.allCompanies[i]._id}
                            onChange={this.addCompany}
                        ></input>
                        <label
                            class="checklistLabels"
                            for={this.state.allCompanies[i].company}
                        >
                            {this.state.allCompanies[i].company}
                        </label>
                    </div>
                );
            } else {
